...
// Data lives in Cloudflare KV (env.HA_KV), seeded from DEFAULTS on first read.
...
export async function onRequest({ request, env }) {
  if (request.method === 'GET') {
    let list;
    try { const raw = await env.HA_KV.get('products:list'); list = raw ? JSON.parse(raw) : null; } catch { /* fall through */ }
  ...
  if (request.method === 'POST' || request.method === 'PUT') {
    ...
    try { await env.HA_KV.put('products:list', JSON.stringify(list)); }
